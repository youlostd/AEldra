...
class RefineDialogNew(ui.ScriptWindow):
	...
	def __LoadScript(self):
		...
		try:
			...
			self.successPercentage = self.GetChild("SuccessPercentage")
			self.fastRefine = self.GetChild("FastRefine")
			self.GetChild("AcceptButton").SetEvent(self.OpenQuestionDialog)
	...
	def Open(self, targetItemPos, nextGradeItemVnum, cost, prob, type, can_fast_refine):
		...
		if not self.IsShow() or not can_fast_refine:
			self.fastRefine.SetChecked(False)

		self.fastRefine.SetVisible(can_fast_refine)
		...
		item.SelectItem(1, 2, nextGradeItemVnum)
		...
		if self.IsShow() and self.fastRefine.IsChecked() and isRefined:
	...
	def SetSearchShopFunc(self, func):
		self.startShopSearchFunc = func
	...
	def AppendMaterial(self, vnum, count):
		...
		if player.GetItemCountByVnum(vnum) >= count:
			textLine.SetPackedFontColor(uiToolTip.ToolTip.POSITIVE_COLOR)
		else:
			textLine.SetPackedFontColor(uiToolTip.ToolTip.NEGATIVE_COLOR)
			thinBoard.SetMouseLeftButtonDownEvent(self.__SearchItem, vnum)
		...
		self.dialogHeight += thinBoard.GetHeight() + 5
		self.UpdateDialog()

	def __SearchItem(self, vnum):
		if self.startShopSearchFunc:
			self.startShopSearchFunc(vnum)

	def UpdateDialog(self):
		newWidth = self.toolTip.GetWidth() + 60
		newHeight = self.dialogHeight + 74
	...
	def OpenQuestionDialog(self, onlyQuestDlg = False):
		...
		dlgQuestion.SetReturnKeyAvail(True)
	...
	def Accept(self):
		net.SendRefinePacket(self.targetItemPos, self.type, self.fastRefine.IsChecked())
		self.isRefined = True
		if not self.fastRefine.IsChecked():
			self.Close()
