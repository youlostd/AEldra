...
class JigsawIcon( ui.ScriptWindow ):
	...
	def LoadWindow( self ):
		...
		try:
			pyScrLoader = ui.PythonScriptLoader( )
			pyScrLoader.LoadScriptFile( self, "UIScript/jigsawiconwindow.py" )

		except:
			import exception
			exception.Abort( "jigsawiconwindow.LoadDialog.LoadScript" )

		try:
			self.gameButton = self.GetChild( "jigsaw_button" )

		except:
			import exception
			exception.Abort("jigsawiconwindow.LoadDialog.BindObject")
		
	def SetOnClickGame( self, fn ):
		self.gameButton.SetEvent( fn )
...
class FishEventGameWaitingPage(ui.ScriptWindow):
	...
	def Show(self):
		...
		self.descIndex = event.RegisterEventSet( "{}/jigsaw/desc.txt".format( app.GetLocalePath( ) ) )
		event.SetEventSetWidth( self.descIndex, self.descBoard.GetWidth( ) - 10 * 2 )
	...
	def OnUpdate(self):
		...
		event.UpdateEventSet( self.descIndex, xposEventSet + 7, -( yposEventSet + 4 - ( 16 * event.GetVisibleStartLine( self.descIndex ) ) ) )
...
class FishEventGamePage(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:				
			...
			self.GetChild("forum_button").SetEvent(ui.__mem_func__(self.__ClickForumButton))
	...
	def __ClickForumButton(self):
		if player.SLOT_TYPE_FISH_EVENT == mouseModule.mouseController.GetAttachedType():
			return
			
		# fishpuzzle thread forum
		app.ShellExecute( constInfo.EVENTS[ 5 ][ 1 ], False )
	...
	def __AddAccept(self, slotIndex):
		...
		net.SendChatPacket('/SendFishShapeAdd %i 0' % slotIndex)
		self.currentShape = 0
		self.DeattachObject()
	...
	def __UseAccept(self, type):
		self.__QuestionCancel()
		if self.fishBoxSlotPos[type] != -1:
			net.SendChatPacket('/SendFishBoxUse %i %i' % (player.INVENTORY, self.fishBoxSlotPos[type]))
	...
	def __ScoreTextEffectEndFrameEvent(self):
		...
		item.SelectItem(1, 2, self.rewardVnum)
