...
class QuestDialog(ui.ScriptWindow):
	...
	def ClearMySelf(self):
		...
		for f in QuestCurtain.OnDoneEventList:
			apply(f,(QuestCurtain,))
	...
	def GetQuestButtonMaxNum(self):
		yStart = self.sy+event.GetLineCount(self.descIndex)*16+20+5
		yEnd = self.board.GetHeight()

		maxButtonNum = max(1, min((yEnd - yStart) / 32, self.QUEST_BUTTON_MAX_NUM))
		return maxButtonNum
	...
	def AppendQuestion(self, name, idx):  # idx는 0부터 시작함. PythonEventManager.cpp line 881 참고. by 김준호
		...
		Showing_button_idx = (cur_questpage_number-1)* self.GetQuestButtonMaxNum()
		
		if Showing_button_idx <= idx and idx < Showing_button_idx + self.GetQuestButtonMaxNum():
			self.btnAnswer[idx].Show()
		else:
			self.btnAnswer[idx].Hide()
		if entire_questbutton_number > self.GetQuestButtonMaxNum():
			self.nextbutton.Show()
