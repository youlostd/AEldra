...
class CombatZoneWindow(ui.ScriptWindow):
	...
	def Initialize(self):
		...
		try:
			...
			self.infoBtn = GetObject("InfoButton")
		...
		self.Hide()
		self.classDataDays = None

	def __InfoButtonClick(self):
		from os import system
		import constInfo
		system("start %s" % "https://board.%s/threads/en-combat-zone.9733/" % constInfo.DOMAIN)
	...
	def OnClickToggleParticipate(self):
		...
		popup.SetText(localeInfo.COMBAT_ZONE_TEAMING_WARNING_BAN)
		popup.SetAcceptEvent(self.OnClickToggleParticipateEvent)
		popup.Open()
		self.popupBanWarning = popup

	def OnClickToggleParticipateEvent(self):
	...
	def OnClickToggleRanking(self, selectedPage):
		if app.GetTime() <= self.ranking_fetch_cooldown:
			return
		self.ranking_fetch_cooldown = app.GetTime() + 1.5
	...
	def SetDescriptionEvent(self):
		...
		event.SetEventSetWidth(self.descIndex, 140)
	...
	def Destroy(self):
		...
		self.main = {}
		self.popupBanWarning = None
		if self.classDataDays:
			self.classDataDays.Destroy()
			self.classDataDays = None
