...
if app.ARABIC_LANG:
	# LOCALE_ARABIC = app.GetLanguage() == app.LANG_ARABIC
	LOCALE_ARABIC = False
else:
	LOCALE_ARABIC = localeInfo.IsARABIC()

CORRECT_WIDTH = 0 if __SERVER__ == 1 else 20
...
if constInfo.PREMIUM_CHAT_COLOR_ENABLED:
	class ChatColorMenu(ui.ImageBox):

		PREMIUM_CHAT_COLORS = [
			ui.GenerateColor(121, 219, 133), # GREEN
			ui.GenerateColor(0, 131, 255), # BLUE
			ui.GenerateColor(255, 242, 0), # YELLOW
			ui.GenerateColor(255, 139, 45),  # ORANGE
			ui.GenerateColor(0, 255, 0),		 # GREEN
		]
...
class ChatLine(ui.EditLine):
	...
	CHAT_MODE_NAME = {	chat.CHAT_TYPE_TALKING : localeInfo.CHAT_NORMAL,
						...
						chat.CHAT_TYPE_TEAM : localeInfo.CHAT_TEAM, }
	...
	def ChangeChatMode(self):
		if chat.CHAT_TYPE_TALKING == self.GetChatMode():
			...
			if player.IsGameMaster():
				self.SetChatMode(chat.CHAT_TYPE_TEAM)
				self.SetText("*")
				self.SetEndPosition()
			else:
		...
		elif chat.CHAT_TYPE_TEAM == self.GetChatMode():
			self.SetChatMode(chat.CHAT_TYPE_TALKING)
			self.SetText("")
	...
	def __SendTeamChatPacket(self, text):

		if 1 == len(text):
			self.RunCloseEvent()
			return

		self.__SendChatPacket(text[1:], chat.CHAT_TYPE_TEAM)
		self.__ResetChat()

	def __SendTalkingChatPacket(self, text):
		if constInfo.FIX_CH_COMMAND:

			if '/' == text[0]:
				if text.startswith("/ch") and len(text) <= 8:
					channelNumStr = text[-1:]
					if len(channelNumStr) != 1 or not channelNumStr.isdigit() or int(channelNumStr) > 5 or int(channelNumStr) < 1:
						chat.AppendChat(1, "Invalid channel number specified!")
						self.__ResetChat( )
						return

					# replace the CMD now
					channelNum = int( channelNumStr )
					if str("channel%i" % ( channelNum )) in constInfo.SERVER_INFO[ "data" ]:
						text = "/fast_change_channel %d %s" % ( channelNum, constInfo.SERVER_INFO[ "data" ][ "channel%i" % ( channelNum ) ][ 0 ] )
						cfg.Set( cfg.SAVE_GENERAL, "channel_info", channelNum )

						chat.AppendChat(1, "Changing channel to %d..." % channelNum)
					else:
						chat.AppendChat(1, "Unable to change channel.")
	...
	def __CheckChatMark(self):
		...
		if len(text) > 0:
			if '#' == text[0]:
				...
				self.overTextLine.SetText("!")
				self.overTextLine.Show()
			elif '*' == text[0] and player.IsGameMaster():
				self.overTextLine.SetText("*")
				self.overTextLine.Show()
	...
	def __PrevLastSentenceStack(self):
		...
		if constInfo.LAST_CHAT_LINES and self.lastSentencePos < len(constInfo.LAST_CHAT_LINES):
			self.lastSentencePos += 1
			lastSentence = constInfo.LAST_CHAT_LINES[-self.lastSentencePos]
	...
	def __NextLastSentenceStack(self):
		...
		if constInfo.LAST_CHAT_LINES and self.lastSentencePos > 1:
			self.lastSentencePos -= 1
			lastSentence = constInfo.LAST_CHAT_LINES[-self.lastSentencePos]
			self.SetText(lastSentence)				
			self.SetEndPosition()			
	...
	def __PushLastSentenceStack(self, text):		
		...
		if len(constInfo.LAST_CHAT_LINES) > LAST_SENTENCE_STACK_SIZE:
			constInfo.LAST_CHAT_LINES.pop(0)

		constInfo.LAST_CHAT_LINES.append(text)
	# END_OF_LAST_SENTENCE_STACK
	...
	def OnIMEReturn(self):
		...
		if (textLen > 0) and (textLen != textSpaceCount):
			if '#' == text[0]:
				...
				self.__SendTeamChatPacket(text)
			else:
...
class ChatInputSet(ui.Window):
	...
	def RefreshPosition(self):
		...
		self.btnSend.SetPosition(self.GetWidth() - 25 - 25, 2)
...
class ChatWindow(ui.Window):
	...
	def __init__(self):
		...
		chatInputSet.SetSize(550 + CORRECT_WIDTH, 25)
		...
		if constInfo.PREMIUM_CHAT_COLOR_ENABLED:
			...
			btnChatColor = ui.Button()
			btnChatColor.SetParent(self)
			btnChatColor.SetUpVisual("d:/ymir work/ui/chat/color_icon.tga")
			btnChatColor.SetOverVisual("d:/ymir work/ui/chat/color_icon.tga")
			btnChatColor.SetDownVisual("d:/ymir work/ui/chat/color_icon.tga")
			# btnChatColor.SetDisableVisual("d:/ymir work/ui/chat/color_icon_disabled.tga")
			btnChatColor.SetDisableVisual("d:/ymir work/ui/chat/color_icon.tga")
			btnChatColor.SetEvent(ui.__mem_func__(self.ToggleChatColorMenu))
			btnChatColor.Hide()
			self.btnChatColor = btnChatColor
	...
	def __RegisterChatColorDict(self):
		CHAT_COLOR_DICT = {
			...
			chat.CHAT_TYPE_TEAM : colorInfo.CHAT_RGB_TEAM,
	...
	def OpenChat(self):
		...
		self.btnSendWhisper.SetPosition(self.GetWidth() - 75 + CORRECT_WIDTH, 2)
		self.btnSendWhisper.Show()

		self.btnChatLog.SetPosition(self.GetWidth() - 50 + CORRECT_WIDTH, 2)
		self.btnChatLog.Show()

		if constInfo.PREMIUM_CHAT_COLOR_ENABLED:
			self.btnChatColor.SetPosition(self.GetWidth() - 25, 2)
			self.btnChatColor.Show()

			(x, y) =  self.btnChatColor.GetGlobalPosition()
			self.chatColorMenu.SetPosition(x + self.btnChatColor.GetWidth()/2 - 21, y - 118)
			
			if constInfo.AUCTION_PREMIUM:
				self.btnChatColor.Enable()
				self.btnChatColor.ClearToolTip()
				self.btnChatColor.SetToolTipText(localeInfo.CHAT_COLOR_PALETTE)
	...
	def OnUpdate(self):
		if constInfo.PREMIUM_CHAT_COLOR_ENABLED:
			if self.btnChatColor.IsIn():
				self.btnChatColor.ShowToolTip()
			else:
				self.btnChatColor.HideToolTip()
...
class ChatLogWindow(ui.Window):
	...
	BLOCK_WIDTH = 32
	CHAT_MODE_NAME = ( localeInfo.CHAT_NORMAL, localeInfo.CHAT_PARTY, localeInfo.CHAT_GUILD, localeInfo.CHAT_SHOUT, localeInfo.CHAT_TEAM, localeInfo.CHAT_INFORMATION, localeInfo.CHAT_NOTICE, )
