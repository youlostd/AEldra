...
class CardsInfoWindow(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		try:
			...
			import exception
			exception.Abort("CardsInfoWindow.DescriptionBox.LoadDialog.BindObject")
	...
	def __OnStartQuestion(self):
		questionDialog = uiCommon.QuestionDialogMultiLine()
		message = localeInfo.MINI_GAME_RUMI_START_QUESTION % (50000, 1)
		message = "{}[ENTER]{}".format( message, localeInfo.MINI_GAME_RUMI_START_QUESTION2 )
		questionDialog.SetText( message )
	...
	def OnUpdate(self):
		...
		event.UpdateEventSet(self.descIndex, xposEventSet+7, -(yposEventSet+7-(self.scrollPos * 16)))
		self.descriptionBox.SetIndex(self.descIndex)
	 
	def OnScroll(self):
		pos = self.scrollBar.GetPos()
		self.scrollPos = int(pos * max(0, event.GetLineCount(self.descIndex)+1 - 18)) # line count +1 => last line is not counted because it's not in the array
		event.SetVisibleStartLine(self.descIndex, self.scrollPos)
		event.Skip(self.descIndex)

	def __SetDescriptionEvent(self):
		event.ClearEventSet(self.descIndex)
		self.descIndex = event.RegisterEventSet("%s/mini_game_okey_desc.txt" % app.GetLocalePath())
		#event.SetRestrictedCount(self.descIndex, 100)
		event.SetEventSetWidth(self.descIndex, self.textBoard.GetWidth() - 7 * 2)
...
class CardsWindow(ui.ScriptWindow):
	...
	def __init__(self):
		ui.ScriptWindow.__init__(self)
		self.safemode = 0
		self.rumiCatalog = "rumi"
		self.questionDialog = None

	def __del__(self):
		ui.ScriptWindow.__del__(self)

	def ChangeDesign(self, eventType):
		
		# Normal design: dont replace anything
		...
		self.rumiCatalog = self.NEW_DESIGNS[ eventType ]

		self.CARDS_ICONS = { 
			1 : "d:/ymir work/ui/minigame/{}/card/card_blue_%d.sub".format(self.rumiCatalog),
			2 : "d:/ymir work/ui/minigame/{}/card/card_red_%d.sub".format(self.rumiCatalog),
			3 : "d:/ymir work/ui/minigame/{}/card/card_yellow_%d.sub".format(self.rumiCatalog), 
		}

		self.background.LoadImage("d:/ymir work/ui/minigame/{}/rumi_bg.tga".format(self.rumiCatalog))

		self.deck_flush_effect.ClearImages()

		images = {
			"deck/deck_flash/deck_card_flush2.sub",
			"deck/deck_flash/deck_card_flush3.sub",
			"deck/deck_flash/deck_card_flush4.sub",
			"deck/deck_flash/deck_card_flush5.sub",
			"deck/deck_flash/deck_card_flush4.sub",
			"deck/deck_flash/deck_card_flush3.sub",
			"deck/deck_flash/deck_card_flush2.sub",
			"deck/deck_flash/deck_card_flush1.sub",
		}

		for image in images:
			self.deck_flush_effect.AppendImage("d:/ymir work/ui/minigame/{}/{}".format(self.rumiCatalog, image))
		
		self.deck_flush_effect.SetDelay(8)
	...
	def __EndGame(self):
		questionDialog = uiCommon.QuestionDialogMultiLine()
		message = "{}[ENTER]{}".format( localeInfo.MINI_GAME_RUMI_EXIT_QUESTION, localeInfo.MINI_GAME_RUMI_EXIT_QUESTION2 )
		questionDialog.SetText( message )
	...
	def UpdateDeckSlot(self):
		if int(self.cards_count.GetText()) > 16:
			self.deckSlot.SetCardSlot(2, 10, "d:/ymir work/ui/minigame/{}/deck/deck3.sub".format(self.rumiCatalog))
			self.deckSlot.SetCardSlot(1, 10, "d:/ymir work/ui/minigame/{}/deck/deck2.sub".format(self.rumiCatalog))
			...
			self.deckSlot.SetCardSlot(0, 10, "d:/ymir work/ui/minigame/{}/deck/deck1.sub".format(self.rumiCatalog))
		else:
...
class IngameWindow(ui.ScriptWindow):
	...
	RUMI_DESIGN_NORMAL 		= 1
	RUMI_DESIGN_HALLOWEEN 	= 2
	...
	def ChangeDesign(self, eventType):
		...
		constInfo.RUMI_EVENT_DESIGN_TYPE = eventType

		# set halloween images
		if eventType == self.RUMI_DESIGN_HALLOWEEN:
			self.gameButton.SetDownVisual("d:/ymir work/ui/minigame/halloween_rumi/rumi_button_min.sub")
			self.gameButton.SetUpVisual("d:/ymir work/ui/minigame/halloween_rumi/rumi_button_min.sub")
			self.gameButton.SetOverVisual("d:/ymir work/ui/minigame/halloween_rumi/rumi_button_min.sub")
