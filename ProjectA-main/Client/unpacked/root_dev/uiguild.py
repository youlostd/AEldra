...
if app.ENABLE_GUILDRENEWAL_SYSTEM:
	...
	class DeclearGuildWarSelectDialog(ui.ScriptWindow):
		...
		def __CreateDialog(self):
			...
			for i in xrange(len(self.WarTypeButtonList)):
			...
			if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
				...
				self.WarTypeButtonList[1].SetShowToolTipEvent(ui.__mem_func__(self.OverInToolTipButton), localeInfo.GUILD_WAR_TYPE_FLAG)
				self.WarTypeButtonList[1].SetHideToolTipEvent(ui.__mem_func__(self.OverOutToolTipButton))
				...
				self.WarTypeButtonList[2].SetShowToolTipEvent(ui.__mem_func__(self.OverInToolTipButton), localeInfo.GUILD_WAR_TYPE_TIME)
		...
		def __OnTypeButtonClick(self, index):
			...
			self.parent.SetGuildWarType(index)
			self.Close()
...
#			#	self.warwinButtonList[i].SAFE_SetEvent(self.__OnClickTypeWarWin,i+1,self.warwinButtonList)
	...
	if app.ENABLE_GUILDRENEWAL_SYSTEM:
		...
		def __OnClickWarTypeSelectButton(self):
			self.WarTpyeSelect = DeclearGuildWarSelectDialog(self)
...
class AcceptGuildWarDialog(ui.ScriptWindow):
	...
	def __CreateDialog(self):
		...
		if True:#app.ENABLE_GUILDRENEWAL_SYSTEM==0:
			self.typeButtonList[0].SAFE_SetEvent(self.__OnClickTypeButtonNormal)
			self.typeButtonList[1].SAFE_SetEvent(self.__OnClickTypeButtonWarp)
			self.typeButtonList[2].SAFE_SetEvent(self.__OnClickTypeButtonCTF)
...
class GuildWindow(ui.ScriptWindow):
	...
	def __MakeInfoPage(self):
		...
		try:
			...
			if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
				...
				# hide guild bonus panel for now
				page.GetChild("Guild_Buff_Title_Bar").Hide()
	...
	def __MakeMemberPage(self):
		...
		if app.ENABLE_GUILDRENEWAL_SYSTEM:			
			...
			btn = page.GetChild("MemberOutButton")
			btn.SetPosition(self.GetWidth() / 2 - btn.GetWidth() / 2, btn.GetLocalPosition()[1])
			...
			page.GetChild("MasterChangeButton").Hide()
			...
			page.GetChild("VoteCheckButton").Hide()
	...
	def __MakeGradePage(self):
		...
		if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
			...
			if app.ENABLE_MELEY_LAIR_DUNGEON:
				page.MeleyAuthority = page.GetChild("MeleyAuthority")
				page.MeleyAuthority.OnMouseOverIn = lambda arg = localeInfo.GUILDWINDOW_GRADE_MELEY : self.OverInToolTipButton(arg)
				page.MeleyAuthority.OnMouseOverOut = lambda : self.OverOutToolTipButton()
		...
		for i in xrange(15):
			...
			if localeInfo.IsARABIC():
				...
				if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
					gradeNumberSlotImage = ui.MakeImageBox(page, "d:/ymir work/ui/public/Parameter_Slot_00.sub", 14-20-15, yPos)
				else:
			...
			if localeInfo.IsARABIC():
				...
				if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
					gradeNameSlot = EditableTextSlot(page, 58-20-15, yPos)
				else:
			...
			if app.ENABLE_GUILDRENEWAL_SYSTEM:
				...
				if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
					if localeInfo.IsARABIC():
						...
						inviteAuthorityCheckBox = CheckBox(page, 122+10-15, yPos, event)
				else:
				...
				if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
					if localeInfo.IsARABIC():
						...
						driveoutAuthorityCheckBox = CheckBox(page, 162+10-15, yPos, event)
				else:
				...
				if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
					if localeInfo.IsARABIC():
						...
						noticeAuthorityCheckBox = CheckBox(page, 202+10-15, yPos, event)
				else:
				...
				if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
					if localeInfo.IsARABIC():
						...
						skillAuthorityCheckBox = CheckBox(page, 242+10-15, yPos, event)
				else:
					skillAuthorityCheckBox = CheckBox(page, 245, yPos, event)
				page.Children.append(skillAuthorityCheckBox)
				
				## War Authority
				event = lambda argSelf=proxy(self), argIndex=index, argAuthority=(1<<4) | (1 <<5): apply(argSelf.OnCheckAuthority, (argIndex,argAuthority))
				if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
					if localeInfo.IsARABIC():
						warAuthorityCheckBox = CheckBox(page, 24, yPos, event)
					else:
						warAuthorityCheckBox = CheckBox(page, 282+10-15, yPos, event)
				else:
					warAuthorityCheckBox = CheckBox(page, 280, yPos, event)
				page.Children.append(warAuthorityCheckBox)
				
				## Bank Authority
				event = lambda argSelf=proxy(self), argIndex=index, argAuthority=(1<<6) | (1 << 7): apply(argSelf.OnCheckAuthority, (argIndex,argAuthority))
				if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
					if localeInfo.IsARABIC():
						bankAuthorityCheckBox = CheckBox(page, -15, yPos, event)
					else:
						bankAuthorityCheckBox = CheckBox(page, 322+10-15, yPos, event)
				else:
				...
				if app.ENABLE_MELEY_LAIR_DUNGEON:
					## Meley Authority
					event = lambda argSelf=proxy(self), argIndex=index, argAuthority=guild.GUILD_AUTH_MELEY_ALLOW: apply(argSelf.OnCheckAuthority, (argIndex,argAuthority))
					if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
						if localeInfo.IsARABIC():
							meleyAuthorityCheckBox = CheckBox(page, -15, yPos, event)
						else:
							meleyAuthorityCheckBox = CheckBox(page, 362+10-15, yPos, event)
					else:
						meleyAuthorityCheckBox = CheckBox(page, 320, yPos, event)
					page.Children.append(meleyAuthorityCheckBox)
				...
				if app.ENABLE_MELEY_LAIR_DUNGEON:
					gradeSlotList.append(meleyAuthorityCheckBox)
	...
	def RefreshGuildMemberPage(self):
		...
		if (guild.GetGuildMasterName() == player.GetName() or test_server) and len(constInfo.GUILD_MEMBERS_LASTPLAYED) == 0 and guild.GetGuildLevel() >= 20:
			net.SendChatPacket('/guild_info')

	def OnMouseWheel(self, len):
		page = self.pageWindow["MEMBER"]
		lineCount = guild.GetMemberCount()
		if self.IsInPosition():
			dir = constInfo.WHEEL_TO_SCROLL(len)
			new_pos = page.scrollBar.GetPos() + ((1.0 / lineCount) * dir)
			new_pos = max(0.0, new_pos)
			new_pos = min(1.0, new_pos)
			page.scrollBar.SetPos(new_pos)
			return True
		return False
	...
	def RefreshGuildMemberPageLastPlayed(self):
		...
		for key, slotList in page.memberDict.items():
			...
			if len(constInfo.GUILD_MEMBERS_LASTPLAYED) > 0 and pid in constInfo.GUILD_MEMBERS_LASTPLAYED:
				slotList[0].OnMouseOverIn = lambda arg = constInfo.GUILD_MEMBERS_LASTPLAYED[pid] : self.OverInToolTipButton(arg)
				slotList[0].OnMouseOverOut = lambda : self.OverOutToolTipButton()
	...
	def RefreshGuildGradePage(self):
		...
		for key, slotList in page.gradeDict.items():
			...
			if app.ENABLE_MELEY_LAIR_DUNGEON:
				slotList[self.GRADE_MELEY_AUTHORITY].SetCheck(authority & guild.GUILD_AUTH_MELEY_ALLOW)

			if app.ENABLE_GUILDRENEWAL_SYSTEM:
				# NOT_IMPLEMENTED_YET
				slotList[self.GRADE_WAR_AUTHORITY].SetCheck(authority & (guild.GUILD_AUTH_SAFEBOX_ITEM_GIVE | guild.GUILD_AUTH_SAFEBOX_ITEM_TAKE))
				slotList[self.GRADE_BANK_AUTHORITY].SetCheck(authority & (guild.GUILD_AUTH_SAFEBOX_GOLD_GIVE | guild.GUILD_AUTH_SAFEBOX_GOLD_TAKE))
	...
	if app.ENABLE_GUILDRENEWAL_SYSTEM:
		...
		def RefreshGuildBaseInfoPageBankGold(self):
			...
			page = self.pageWindow["BASE_INFO"]
			# NOT_IMPLEMENTED_YET
			gold, lastgolduseuser, lastbankuseuser = (0, "NOT_IMPLEMENTED", "NOT_IMPLEMENTED")#guild.GetBaseInfoBankGold()
		...
		def RefreshGuildBaseInfoPage(self):
			...
			# NOT_IMPLEMENTED_YET
			local, banklevel, = (0,0)# guild.GetBaseInfo()
			...
			# NOT_IMPLEMENTED_YET
			if True:#guild.GetbuildingSize()==0:
				page.powerlevelSlot.SetText("")
				page.factorySlot.SetText("")
				page.smelterSlot.SetText("")
				page.banklevelSlot.SetText("")
		...
		def __OnOutMemberStart(self):
			...
			if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
				if page.Name != "":
					...
					m2netm2g.SendGuildRemoveMemberPacket(page.Name)
					tchat(str(page.Name))
					if self.popup:
						self.popup.Close()
						self.popup = None
...
if False: #app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
...
class BuildGuildBuildingWindow(ui.ScriptWindow):
	...
	def __CreateBuildInInstance(self,race):
		...
		X_SIZE_STEP = 2 * 2 ## Must be increased only in units of 2
		Y_SIZE_STEP = 8
	...
	def __EnablePCBlocker(self):
		## PC Blocker Turn on processing. (Transparent)
	...
	def __DisablePCBlocker(self):
		## PC Blocker Turn off processing. (Not transparent)
		chr.SetInstanceType(chr.INSTANCE_TYPE_OBJECT)
