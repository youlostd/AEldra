...
HAIR_SELECT_LIST = {
	JOB_WARRIOR : list(xrange(73001, 73012 + 1)),
	JOB_ASSASSIN : list(xrange(73251, 73262 + 1)),
	JOB_SURA : list(xrange(73501, 73512 + 1)),
	JOB_SHAMAN : list(xrange(73751, 73762 + 1)),
}
if app.ENABLE_WOLFMAN_CHARACTER:
	HAIR_SELECT_LIST[JOB_WOLFMAN] = []

class MyCharacters :
	class MyUnit :

		def __init__(self, const_id, name, level, race, playtime, guildname, form, hair, hair_base, acce, stat_str, stat_dex, stat_hth, stat_int, change_name):
			self.UnitDataDic = {
				"ID" 	: 	const_id,
				"NAME"	:	name,
				"LEVEL"	:	level,
	...
	def LoadCharacterData(self) :
		...
		for i in xrange(net.CHARACTER_SLOT_COUNT_MAX) :
			...
			hair_base		= net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_HAIR_BASE)
			...
			if constInfo.CHECK_WRONG_CHARACTER and name not in constInfo.CHARACTER_LIST:
				constInfo.CHARACTER_LIST.append(name)

			self.SetPriorityData(last_playtime, i)

			self.myUnitDic[i] = self.MyUnit(i, name, level, race, playtime, guildname, form, hair, hair_base, acce, stat_str, stat_dex, stat_hth, stat_int, change_name)
	...
	def MakeCharacter(self, slot, name, race, form, hair, acce):
		...
		## ��������, ������ ���� ���.
		if app.ENABLE_WOLFMAN_CHARACTER and chr.RaceToJob(race) == JOB_WOLFMAN:
	...
	def GetHairBase(self, slot):
		try:
			return self.myUnitDic[slot].GetUnitData()["HAIR_BASE"]
		except:
			return 0

	def SetHairBase(self, slot, realSlot, hairBase):
		try:
			unitData = self.myUnitDic[slot].GetUnitData()
			if unitData["HAIR_BASE"] == hairBase:
				return

			unitData["HAIR_BASE"] = hairBase
			chr.SelectInstance(slot)
			if hairBase == 0:
				oldHairBase = unitData["HAIR_BASE_SAVE"]
				hair = unitData["HAIR"]
				if oldHairBase != 0:
					item.SelectItem(1, 2, oldHairBase)
					if item.GetValue(3) == hair:
						hair = 0
				chr.SetHair(hair)
			else:
				item.SelectItem(1, 2, hairBase)
				chr.SetHair(item.GetValue(3))

			net.SendSelectCharacterHairPacket(realSlot, hairBase)
		except:
			pass
...
class SelectCharacterWindow(ui.Window) :	
	...
	def Open(self):
		...
		##HairSelect UI##
		self.hairSelectPrev = getChild("hairselect_prev")
		self.hairSelectPrev.SAFE_SetEvent(self.SetHairBasePrev)
		self.hairSelectNext = getChild("hairselect_next")
		self.hairSelectNext.SAFE_SetEvent(self.SetHairBaseNext)
		self.hairSelectImg = getChild("hairselect_image")
		self.hairSelectIdx = -1
		...
		if os.path.isfile("select.txt") and not app.IsPressed(app.DIK_LCONTROL):
			net.DirectEnter(0)

		self.SetFocus()

		if constInfo.RELOGIN_LOGIN_WINDOW and constInfo.RELOGIN_TRY_LOGIN and constInfo.RELOGIN_PICK_CHARATER:
	...
	def SelectButton(self, slot):		
		...
		event.SetEventSetWidth(self.descIndex, self.textBoard.GetWidth() - 10)
		event.SetVisibleLineCount(self.descIndex, self.VISIBLE_DESC_LINE_COUNT)
		
		self.eventPageIdx = 0
		self.eventPageCount = 1

		self.ResetStat()
		self.RefreshHairSelect()
	...
	def Close(self):
		...
		self.statValue = None#[]
		self.GaugeList = None#[]
		self.hairSelectPrev = None
		self.hairSelectNext = None
		self.hairSelectImg = None
		self.textBoard = None
		self.btnPrev = None
		self.btnNext = None
	...
	def StartGameButton(self):
		...
		if constInfo.WHISPER_MANAGER:
			if not os.path.exists("temp"):
				os.mkdir("temp")
			if not os.path.exists("temp/whisper"):
				os.mkdir("temp/whisper")
			import chat
			chat.Clear()
			constInfo.IS_CHAT_CLEARED = True
			whisperFileName = net.GetAccountCharacterSlotDataString(self.stream.GetCharacterSlot(), net.ACCOUNT_CHARACTER_SLOT_NAME)
			whispermgr.SetFileName("temp/whisper/" + whisperFileName)

		if musicInfo.selectMusic != "":
			snd.FadeLimitOutMusic("BGM/"+musicInfo.selectMusic, systemSetting.GetMusicVolume()*0.05)

		net.DirectEnter(self.stream.GetCharacterSlot())
		playTime = net.GetAccountCharacterSlotDataInteger(self.stream.GetCharacterSlot(), net.ACCOUNT_CHARACTER_SLOT_PLAYTIME)
	...
	def OnUpdate(self):
		...
		# if self.MotionStart and app.GetTime() - self.MotionTime >= 0.0 :
		# 	self.MotionStart = False
		# 	##���� ��� �״�� �鱸��##
		# 	#print " Start Game "
		...
		self.btnPrev.SetVisible(event.GetVisibleStartLine(self.descIndex) > 0)
		self.btnNext.SetVisible(event.IsLocalWait(self.descIndex) or (self.eventPageIdx < (self.eventPageCount-1)))
	...
	def __GetHairSelectList(self):
		return HAIR_SELECT_LIST[self.mycharacters.GetJob(self.SelectSlot)]

	def RefreshHairSelect(self):
		hairBase = self.mycharacters.GetHairBase(self.SelectSlot)
		list = self.__GetHairSelectList()
		if hairBase in list:
			self.SelectHairBase(list.index(hairBase))
		else:
			self.SelectHairBase(-1)

	def SetHairBaseNext(self):
		list = self.__GetHairSelectList()
		idx = self.hairSelectIdx + 1
		if idx >= len(list):
			idx = -1
		self.SelectHairBase(idx)

	def SetHairBasePrev(self):
		list = self.__GetHairSelectList()
		idx = self.hairSelectIdx - 1
		if idx < -1:
			idx = len(list) - 1
	...
	def SelectHairBase(self, idx):
		list = self.__GetHairSelectList()
		self.hairSelectIdx = idx
		vnum = 0

		if idx == -1:
			item.SelectItem(1, 2, list[0])
			self.hairSelectImg.LoadImage(item.GetIconImageFileName())
			self.hairSelectImg.Show()
		else:
			item.SelectItem(1, 2, list[idx])
			self.hairSelectImg.LoadImage(item.GetIconImageFileName())
			self.hairSelectImg.Show()
			vnum = list[idx]

		self.mycharacters.SetHairBase(self.SelectSlot, self.RealSlot[self.SelectSlot], vnum)
	...
	def PrevDescriptionPage(self):
		...
		if True == event.IsWait(self.descIndex) :
			if event.GetVisibleStartLine(self.descIndex) - self.VISIBLE_DESC_LINE_COUNT >= 0:
				event.SetVisibleStartLine(self.descIndex, event.GetVisibleStartLine(self.descIndex) - self.VISIBLE_DESC_LINE_COUNT)
				event.Skip(self.descIndex)
		else :
			event.Skip(self.descIndex)
	
	def NextDescriptionPage(self):
		pageIdx = self.eventPageIdx + 1
		if pageIdx >= self.eventPageCount:
			self.eventPageCount = pageIdx + 1
		self.eventPageIdx = pageIdx
	...
	def DisableWindow(self):
		...
		self.hairSelectPrev.Disable()
		self.hairSelectNext.Disable()
		self.toolTip.Hide()
		self.ShowToolTip = False
		self.Disable = True
		for button in self.CharacterButtonList :
	...
	def EnableWindow(self):
		...
		self.btnPrev.Enable()
		self.btnNext.Enable()
		self.hairSelectPrev.Enable()
		self.hairSelectNext.Enable()
		self.Disable = False
		for button in self.CharacterButtonList :
			button.Enable()
	...
	def AcceptInputName(self):
		...
		if changeName.find(localeInfo.CREATE_GM_NAME)!=-1:
			self.PopupMessage(localeInfo.CREATE_ERROR_GM_NAME, self.EnableWindow)
			return

		if changeName.startswith(' ') or changeName.endswith(' '):
			self.PopupMessage("The name can't start or end with a space.", self.EnableWindow)
			return

		if '  ' in changeName:
			self.PopupMessage("Only one space letter.", self.EnableWindow)
			return False
			
		if not len(set(changeName) & set('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789')): #does not contain any alphanum char
			self.PopupMessage("Use at least 1 normal letter.(a-Z 0-9)", self.EnableWindow)
			return
