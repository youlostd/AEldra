...
class PartyMemberInfoBoard(ui.ScriptWindow):
	...
	MEMBER_BUTTON_IMAGE_FILE_NAME_DICT = {	player.PARTY_STATE_ATTACKER : "party_state_attacker",
	...
	def OnMouseLeftButtonDown(self):
		...
		if self.vid:
			...
			if mouseModule.mouseController.isAttached():
				...
				net.SendExchangeItemAddPacket(mouseModule.mouseController.GetAttachedType(), attachedSlotPos, 0)
	...
	def AddPartyMember(self, pid, name):
		...
		if vid != None:
			if constInfo.ENABLE_DMG_METER:
				self.interface.wndDmgMeter.LogOutPlayer(vid)
	...
	def UpdatePartyMemberInfo(self, pid):
		...
		if None != vid:
			if player.IsMainCharacterIndex(vid):
				# tchat("IsPartyLeaderByPID (%d) %s" % (pid, str(player.IsPartyLeaderByPID(pid))))
				if player.IsPartyLeaderByPID(pid):
